...
[docs/final-report.md](../docs/final-report.md) (actions A-G). Each requirement traces back to a
specific insight and a specific KPI. Nothing here is a generic "best practice" example
...
`credit_application` are truly mandatory (regulatory requirement) versus merely convenient to
collect upfront is a compliance question, not a UX one. Writing a specific requirement before
that review would risk specifying the wrong solution. This is intentional. Not every finding is
ready to become a requirement on day one, and a requirements document should say so rather than
force a premature specification.
