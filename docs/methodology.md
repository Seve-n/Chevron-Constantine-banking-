...
phase produced a standalone, reviewable deliverable before the next one started — deliberately
slower than jumping straight to "build a dashboard," because a Business Analyst's job is the
framing and the traceability between steps, not just the end artifact.
...
2. **No real banking data risk.** Belfius data protection requirements make using any real
   ...
   deliberately injected data-quality noise — duplicates, missing values, casing
   inconsistencies — so the cleaning phase has genuine work to do, not a formality).
...
- **US-007** has no acceptance criteria, for the same reason — writing them would mean
  specifying a solution before compliance confirms it's implementable.
